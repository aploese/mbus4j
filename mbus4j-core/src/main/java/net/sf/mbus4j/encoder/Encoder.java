...
public class Encoder {
	...
	public byte[] encodeFrame(Frame frame) {
		if (frame instanceof RequestFrame<?>) {
			return encodeRequestFrame((RequestFrame<?>) frame);
		} else if (frame instanceof ResponseFrame) {
			return encodeResponseFrame((ResponseFrame) frame);
		} else {
			throw new RuntimeException("Should never happen, cant encode frame.\n" + frame);
		}
	}

	public byte[] encodeRequestFrame(RequestFrame<?> frame) {
		if (frame instanceof SingleCharFrame) {
			return encodeSingleCharFrame((SingleCharFrame) frame);
		} else if (frame instanceof ShortFrame) {
			return encodeShortFrame((ShortFrame<?>) frame);
		} else if (frame instanceof SendUserDataFrame) {
			return encodeSendUserDataFrame((SendUserDataFrame) frame);
		} else {
			throw new RuntimeException("Should never happen");
		}
	}

	public byte[] encodeSendUserDataFrame(SendUserDataFrame frame) {
		initFrame(frame);
		pushCField(frame);
		pushAField(frame);
		if (frame instanceof ApplicationReset) {
			...
			pushVariableDataBlocks((SendUserData)frame);
			...
			throw new RuntimeException("Shoud never happen Cant handle CI request field:\n" + frame);
		}
	...
	public byte[] encodeResponseFrame(ResponseFrame frame) {
		if (frame instanceof SingleCharFrame) {
			return encodeSingleCharFrame((SingleCharFrame) frame);
		} else if (frame instanceof UserDataResponse) {
			return encodeUserDataResponseFrame((UserDataResponse) frame);
		} else if (frame instanceof GeneralApplicationError) {
			return encodeGeneralApplicationErrorFrame((GeneralApplicationError) frame);
		} else {
			throw new RuntimeException("Should never happen");
		}
	}

	private byte[] encodeGeneralApplicationErrorFrame(GeneralApplicationError frame) {
		initFrame(frame);
		pushCField(frame);
		pushAField(frame);
		//CI-Field
		...
		//data
		data[currentPos++] = frame.getError().id;
	...
	private byte[] encodeUserDataResponseFrame(UserDataResponse frame) {
		initFrame(frame);
		pushCField(frame);
		pushAField(frame);
		//CI-Field
	...
	public byte[] encodeShortFrame(ShortFrame<?> frame) {
		initFrame(frame);
		pushCField(frame);
		pushAField(frame);
	...
	public byte[] encodeSingleCharFrame(SingleCharFrame frame) {
		initFrame(frame);
		return data;
	}
	...
	private void initFrame(Frame frame) {
		if (frame instanceof SingleCharFrame) {
			...
			data[0] = 0x10;
			currentPos = 1;
		} else if ((frame instanceof SendUserDataFrame) || (frame instanceof LongFrame)) {
			data = new byte[0xFF];
			data[0] = 0x68;
			data[3] = 0x68;
			currentPos = 4;
	...
	private boolean needDIFE(DataBlock db, int index) {
		return ((db.getStorageNumber() >> (1 + index * 4)) > 0x00) || (((db.getTariff() >> (index * 2)) << 0x04) > 0x00)
	...
	private void pushCField(Frame frame) {
		...
		case SND_UD:
			if (frame instanceof RequestFrame<?>) {
				data[currentPos] = (byte) (0x53 + (((RequestFrame<?>) frame).isFcb() ? 0x20 : 0));
			} else {
				throw new RuntimeException("Cant handle Frame: " + frame);
			}
