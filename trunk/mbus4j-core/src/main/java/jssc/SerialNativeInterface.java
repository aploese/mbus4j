...
public class SerialNativeInterface {
    ...
    static {
        ...
        for (int tries = 0; tries < 100; tries++) {
            ...
            final String extention = tries == 0 ? "" : "_" + tries + " "; 
            String libResName = System.mapLibraryName("jSSC-" + libVersion + "_" + architecture);
            libName = System.mapLibraryName("jSSC-" + libVersion + "_" + architecture + extention);
            ...
            if(isLibFolderExist(libFolderPath)){
                if(isLibFileExist(libFolderPath + fileSeparator + libName)){
                    ...
                    if(extractLib((libFolderPath + fileSeparator + libName), osName, libName, libResName)){
                        loadLib = true;
                    }
                }
            }
     ...
     * @since 0.8
     */
    private static boolean extractLib(String libFilePath, String osName, String libName, String libResName) {
        boolean returnValue = false;
        File libFile = new File(libFilePath);
        InputStream input = null;
        FileOutputStream output = null;
        input = SerialNativeInterface.class.getResourceAsStream("/libs/" + osName + "/" + libResName);
        if(input != null){
            int read;
